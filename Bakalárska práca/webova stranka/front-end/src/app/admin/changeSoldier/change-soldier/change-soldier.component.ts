...
@Component({
  selector: 'app-change-soldier',
  templateUrl: './change-soldier.component.html',
  styleUrls: ['./change-soldier.component.scss']
})

export class ChangeSoldierComponent    implements OnInit {
  displayedColumns = ['_id', 'name', 'dateOfBirth', 'dateOfDeath',  'story', 'addedBy', 'state',  'actions'];
  ...
  constructor(public httpClient: HttpClient,
              ...
              public dataService: DataService) {
                this.meta.addTags([
                  { name: 'change-soldier:card', content: 'change soldier' },
                ]);
              }
  ...
  startEdit(i: number, _id: string, addedBy: string, dateOfBirth: string, dateOfDeath: string, name: string, state: string, story: string) {
    this.id = _id;
    // index row is used just for debugging proposes and can be removed
    this.index = i;

    const dialogRef = this.dialog.open(EditDialogComponent, {
      data: {_id: _id, addedBy: addedBy, dateOfBirth: dateOfBirth, dateOfDeath: dateOfDeath, name: name, state: state, story: story}
    });
  ...
  connect(): Observable<Issue[]> {
    ...
    return merge(...displayDataChanges).pipe(map( () => {
        ...
        this.filteredData = this._exampleDatabase.data.slice().filter((issue: Issue) => {
          const searchStr = (issue._id + issue.addedBy + issue.state + issue.story + issue.dateOfBirth + issue.dateOfDeath + issue.name).toLowerCase();
  ...
  sortData(data: Issue[]): Issue[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case 'story': [propertyA, propertyB] = [a.story, b.story]; break;
